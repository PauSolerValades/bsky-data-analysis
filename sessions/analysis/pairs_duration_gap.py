...
counts the (duration family x gap family) contingency. Users with <= 30 sessions
(n_obs on the duration row) are excluded.

Usage:
    uv run analysis/pairs_duration_gap.py
"""
...
TSV = Path(__file__).resolve().parent.parent / "distribution-fit/results/best_per_user.tsv"
MIN_SESSIONS = 30  # exclude users with <= 30 sessions
...
print(f"users kept (>{MIN_SESSIONS} sessions): {users:,}  (dropped {dropped:,})")
print()
