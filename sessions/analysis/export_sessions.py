...
Exports sessions_all (all events, incl. likes) and sessions_engagement
(engaged events, no likes) into data_new/ directory with consistent columns.

Usage:
    uv run sessions/analysis/export_sessions.py
    uv run sessions/analysis/export_sessions.py --sample 100000
"""
...
def export_table(conn: pymysql.Connection, table: str, output_path: Path):
    ...
    # Set a longer query timeout for large exports
    with conn.cursor() as setup_cur:
        setup_cur.execute("SET query_timeout = 3600")
...
def main():
    ...
    parser.add_argument(
        "--tables", type=str, default="sessions_all,sessions_engagement",
        help="Comma-separated table names (default: sessions_all,sessions_engagement)",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--output-dir", type=str, default="sessions/analysis/data",
