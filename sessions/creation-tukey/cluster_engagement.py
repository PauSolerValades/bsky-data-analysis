...
# ///
"""
Tukey-session clustering — Bluesky firehose → pau_db.sessions_engagement.

Source: pau_db.engaged_events (5 event types — NO likes — ≥4 engaged events
         per user, time-windowed to 2026-04-11 → 2026-04-18).

This answers the question: "when is the user actively creating or curating
content?"  Likes are excluded because they represent passive consumption,
not active production.

Contrast with sessions_all: sessions_all includes likes and answers
"when is the user browsing?"  Different questions, different session
boundaries.

Method (per-user adaptive IQR / Tukey's fences):
  • Read ALL events for each user from pau_db.engaged_events.
    Event types: post_top, post_reply, feed_repost, graph_follow,
    graph_block, plus minor types (feed_threadgate, etc.) which are ignored.
  ...
  • Write to pau_db.sessions_engagement.

Parameters (from new EDA, docs/EDA.md):
  • The engaged_events table already applies the ≥4 events filter
    (power-law xmin from engaged-events distribution, α=1.67).
  • IQR multiplier: 1.5
...
Usage:
    ...
    uv run sessions/creation-tukey/cluster_engagement.py --min-events 4 --max-events 500 --summary
"""
...
class Session:
    ...
    def total_actions(self) -> int:
        return self.posts + self.replies + self.reposts + self.follows + self.blocks


def _inc(s: Session, action_type: str):
    # Map from engaged_events event_type naming (feed_repost, graph_follow, etc.)
    if action_type == "post_top":
        s.posts += 1
        ...
        s.blocks += 1
    # Other types (feed_threadgate, graph_listitem, etc.) are ignored —
    # they're not part of the five core engaged event types.
...
CREATE_TABLE_SQL = """
CREATE TABLE IF NOT EXISTS pau_db.sessions_engagement (
...
INSERT_SQL = """
INSERT INTO pau_db.sessions_engagement
    (did, session_start, session_end, next_session_start,
     duration_s, posts_authored, replies, reposts, follows, blocks,
     total_actions,
...
def fetch_events_for_dids(
    ...
    Reads from pau_db.engaged_events (already time-windowed, ≥4 engaged
    events per user).  Event types use the engaged_events naming convention:
    feed_repost, graph_follow, graph_block, post_top, post_reply,
    plus minor types (feed_threadgate, graph_listitem, etc.) which are ignored.
    """
    if not dids:
        return {}
...
def load_dids_from_db(
    ...
    """Return DIDs with event count in [min_events, max_events] from engaged_events.
...
def main():
    parser = argparse.ArgumentParser(
        description="Tukey-session clustering → pau_db.sessions_engagement"
    )
    parser.add_argument(
        "--min-events", type=int, default=4,
        help="Minimum events per user (default: 4, engaged_events already filters ≥4)",
    )
    parser.add_argument(
    ...
    if not all_dids:
        print("No DIDs to process.  Run EDA/02_create_engaged_events.sql first.",
              file=sys.stderr)
        conn_sr.close()
        sys.exit(1)
    ...
    conn_sr.commit()
    print("Table pau_db.sessions_engagement ready.", file=sys.stderr)
    ...
    if args.summary and all_durations:
        _print_summary(
            all_durations, all_posts, all_replies, all_reposts,
            all_follows, all_blocks, all_total,
...
def _print_summary(
    ...
    header = f"  SESSION ANALYSIS SUMMARY — sessions_engagement  (n={n:,} sessions, {total_users:,} users)"
    print("\n" + "=" * 60, file=sys.stderr)
