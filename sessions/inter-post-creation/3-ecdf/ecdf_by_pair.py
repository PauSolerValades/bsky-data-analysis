...
within-session inter-post gaps (step plot, log-x; the global overlay was
dropped — the within law stands on its own), plus the within-gap sample
export for the simulator.

Default: all pairs with >= 1% of trusted users. --pair dur,gap for one.

Output: plots/interpost_ecdf__<dur>__<gap>.png (one per pair),
        ecdfs/within_ecdf__<dur>__<gap>.txt (250k gaps, one per line).
    ...
    for dur, gap_f, n_users in keep.iter_rows():
        ...
        for col, label, color in (("interpost_within", "within-session", "#0072B2"),):
            ...
            if col == "interpost_within":
                out_txt = HERE / "ecdfs" / f"within_ecdf__{dur}__{gap_f}.txt"
        ...
        ax.set_ylabel("ECDF")
        ax.set_title(f"Within-session inter-post gap ECDF — {DISPLAY[dur]} $\\times$ "
