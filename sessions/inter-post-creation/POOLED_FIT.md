...
1. **Everyone pooled** — all 11.1M within gaps (1M uniform subsample,
   `results/within_interpost_ecdf.txt`).
2. **Per (duration × gap) pair** — users grouped by their settled parametric
   ...
   gaps pooled per pair (the existing `results/within_ecdf__<dur>__<gap>.txt`
...
`uv run inter-post-creation/pooled_fit.py results/within_ecdf__<dur>__<gap>.txt`
...
(`results/within_ecdf__<dur>__<gap>.txt`). A parametric law per pair is
