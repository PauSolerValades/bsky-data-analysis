...
def main():
    ...
    df = pl.read_csv(input_path, null_values=["NA", ""])
    print(f"  → {df.shape[0]:,} rows, {df.shape[1]} cols", file=sys.stderr)

    if "source_table" in df.columns:
        sources = df["source_table"].unique().to_list()
        print(f"  source tables in file: {sources}", file=sys.stderr)
        df = df.filter(pl.col("source_table") == "sessions_tukey")
        print(f"  → {df.shape[0]:,} rows (sessions_tukey only)", file=sys.stderr)
