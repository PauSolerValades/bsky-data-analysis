...
Usage:
    uv run session-analysis/eda.py                    # run everything
    uv run session-analysis/eda.py --force            # re-fetch stats from DB
    uv run session-analysis/eda.py --skip 4           # skip gap analysis (slow)
    uv run session-analysis/eda.py --only 1,2,5       # run only specific sections
    uv run session-analysis/eda.py --gap-sample 30000 # gap analysis sample size
"""
...
OUT_DIR = Path(__file__).resolve().parent / "eda" / "results"

# Import all section modules
from eda import powerlaw_binning as s1
from eda import user_classification as s2
from eda import activity_span as s3
from eda import gap_analysis as s4
from eda import coverage as s5
from eda import event_type_dist as s6
from eda import composite_score as s7
from eda import recommend as s8
