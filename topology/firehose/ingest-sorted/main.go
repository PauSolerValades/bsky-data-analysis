...
`)
	...
	allFiles, err := DiscoverFiles(baseDir)
	if err != nil {
	...
	todo, err := FilterAlreadyParsed(db, allFiles)
	if err != nil {
	...
	for i, fp := range todo {
		...
		count, err := ProcessFile(db, fp, *batchSize)
		if err != nil {
			log.Printf("FAILED: %v", err)
			_ = MarkFileParsed(db, fp, 0, "FAILED")
			continue
		}

		if err := MarkFileParsed(db, fp, count, "COMPLETED"); err != nil {
