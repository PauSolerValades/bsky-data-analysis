"""Log-log histogram — events per active day per user."""
...
data = np.array([float(t) / max(int(d), 1) for t, d in rows])
data = data[data > 0]
...
ax.set_ylabel("Users")
ax.set_title("Events per active day", fontsize=12, fontweight="bold")
...
fig.tight_layout()
path = OUT / "hist_events_per_day.png"
