...
└── src/             # All ingest logic (single package: "src")
...
### `src/db.go` — Database initialisation
...
### `src/discover.go` — File discovery
...
### `src/records.go` — Data types & helpers
...
### `src/parsed.go` — Idempotency tracking
...
Four tables (see `src/schema.sql` and `bluesky_db_specification.md`):
